...
3. The Line Support Options panel opens on the right with Spacing, Delete, Apply, and Close controls. The Support Preset panel is shown underneath it.
...
9. Change Spacing to rebuild the preview markers.
...
- Spacing is a maximum distance between generated guide points. Each segment is divided into enough equal intervals that no interval is longer than Spacing.
...
- `LineSupportSettings` stores the persistent polyline points and spacing in `Pillar.Core.Layers`.
